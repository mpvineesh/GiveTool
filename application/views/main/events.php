			...
			<ul class="newslist">			
				...
				if(count($upcomingevents) > 0){
					...
					foreach ($upcomingevents as $item){
						...
						echo '<h3>No Events in this category</h2>';						
					}
				?>		
...
<h2 class="title"><span>Most Contributed Events</span></h2>
<div class="background-content clearfix">
<ul class="newslist">			
				<?php 
					if(count($mostcontributedevents) > 0){
						$CI =& get_instance(); 
						$url = $CI->config->config['base_url'];	
						foreach ($mostcontributedevents as $item){
							echo '	
								<li>
									<a href="'.$url.'/index.php/main/eventdetails/'.$item->int_event_id.'"><h3>'.$item->str_name.'</h3></a>
									<p>'.$item->str_description.'</p>
			...
			</ul><div class="both-btn clearfix">
